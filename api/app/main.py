...
from app.shared.adapters.database import create_all
from app.shared.adapters.exception_handlers import register_exception_handlers


@asynccontextmanager
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    # Conveniencia de desarrollo: asegura el esquema. En entornos formales el
    # esquema lo gestiona Alembic (alembic upgrade head).
    create_all()
    yield


def create_app() -> FastAPI:
    app = FastAPI(title="indumentaria-pos", version="0.1.0", lifespan=lifespan)
